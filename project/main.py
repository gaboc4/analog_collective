...
tokens_info = "These tokens represent how much money you have put into the \
				platform in order to add your songs to a playlist. You can \
				add more to your account from the Store page!"
...
def profile():
	...
	if code:
		...
		spotify_token = SpotifyToken(user.id, token_info['access_token'],
										token_info['refresh_token'])
		db.session.add(spotify_token)
		db.session.commit()

	access_token = SpotifyToken.query.filter_by(user_id=
												user.id).first().access_token

	if not code:
		access_token = refresh_access_token(
		SpotifyToken.query.filter_by(user_id=user.id).first().refresh_token)
